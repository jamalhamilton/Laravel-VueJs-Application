  ...
  $.widget('ui.table_scroll', {
    ...
    _ensureSettings: function () {
      if (this.options.rowsInHeader == null) {
        if (this._table().tHead) { this.options.rowsInHeader = this._table().tHead.rows.length } else { this.options.rowsInHeader = 1 }
      }

      if (this.options.rowsInFooter == null) {
        if (this._table().tFoot) { this.options.rowsInFooter = this._table().tFoot.rows.length } else { this.options.rowsInFooter = 0 }
      }
    },

    // horisontal scrolling methods
    _xGetNumberOfColumns: function () {
      if (this._columnsCount != -1) { return this._columnsCount }

      this._columnsCount = Math.max.apply(null, $(this._table().rows).map(function () { return this.cells.length }).get())

      if ($('.sg-v-scroll-cell', this.widget()).length > 0) { this._columnsCount -= 1 }
    ...
    _xScrollWidth: function () {
      var width = this._xGetNumberOfColumns() - this.options.fixedColumnsLeft - this.options.fixedColumnsRight
      if (width > this.options.columnsInScrollableArea) { return this.options.columnsInScrollableArea }
      if (width < 1) { return 1 }
      return width
    },
    ...
    _xInitScroll: function () {
      if (this._xGetNumberOfColumns() < (this.options.fixedColumnsLeft + this.options.fixedColumnsRight)) { return }
    ...
    _xColumnScrollStep: function () {
      if (this._xScrollableColumnsCount() == 0) { return 0 }
    ...
    _xMoveScroll: function (position) {
      ...
      var $widthDivContainer = $('.sg-h-scroll-container', this.widget())
      if ($widthDivContainer.scrollLeft() != position) { $widthDivContainer.scrollLeft(position) }
    },
      ...
      for (var rowIndex = start; rowIndex < end; rowIndex++) {
        ...
        for (var cellIndex = 0; cellIndex < row.cells.length; cellIndex++) {
          ...
          if (cIndex == index) {
            ...
            {
              if (visible && cell.style.display == 'none') { cell.style.display = '' }

              if (!visible && cell.style.display != 'none') { cell.style.display = 'none' }
            }
          }
        }
      }
      ...
      for (var i = this.options.rowsInHeader; i < this._table().rows.length - this.options.rowsInFooter - $('.sg-h-scroll-container', this.widget()).length; i++) {
        if ($(this._table().rows[i]).css('display') != 'none') {
          for (var j = this.options.fixedColumnsLeft; j < this._xGetNumberOfColumns() - this.options.fixedColumnsRight; j++) {
            if ($(this._table().rows[i].cells[j]).css('display') != 'none') { return $(this._table().rows[i].cells[j]).width() }
          }
        }
      }
      return 0
    ...
    _yScrollHeight: function () {
      ...
      if ($('.sg-h-scroll-container', this.widget()).length > 0) { height-- }

      if (height > this.options.rowsInScrollableArea) { return this.options.rowsInScrollableArea }
    ...
    _yInitScroll: function () {
      if (this._table().rows.length < (this.options.rowsInHeader + this.options.rowsInFooter)) { return }
    ...
    _yRowScrollStep: function () {
      if (this._yScrollableRowsCount() == 0) { return 0 }
    ...
    _yMoveScroll: function (position) {
      ...
      var $heightDivContainer = $('.sg-v-scroll-container', this.widget())
      if ($heightDivContainer.scrollTop() != position) { $heightDivContainer.scrollTop(position + step / 2) }
    },
    ...
    _tableMouseWheel: function (event) {
      ...
      if (original.detail) {
        if (original.detail == -3) { up = true } else
        if (original.detail == 3) { down = true }
      }

      var $heightDivContainer = $('.sg-v-scroll-container', this.widget())
      var delta = 0

      if (up) { delta = this._yRowScrollStep() + 1 }
      if (down) { delta = -this._yRowScrollStep() - 1 }
    ...
    _setActualCellIndexes: function () {
      ...
      for (var rowIndex = 0; rowIndex < rows.length; rowIndex++) {
        ...
        if (!indAdjustments) { indAdjustments = [] }
    ...
    _getColSpan: function ($cell) {
      if ($cell.data('scroll-span')) { return $cell.data('scroll-span') }

      if ($cell.attr('colspan')) { return $cell.attr('colspan') * 1 }
